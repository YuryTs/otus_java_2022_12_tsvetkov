...
public class DataTemplateHibernate<T> implements DataTemplate<T> {
    ...
    @Override
    public List<T> findByEntityField(Session session, String entityFieldName, Object entityFieldValue) {// имя поля сущности передается и значение этого поля по которому мы фильтруем
