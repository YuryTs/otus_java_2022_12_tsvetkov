...
//         phantomDemo();
    ...
    private static void strong() {
        ...
        for (var idx = 0; idx < size; idx++) {
            references.add(new BigObject()); //создаем коллекцию больших объектов через new
            logger.info("idx:{}", idx);
        }
    ...
    private static void weak() {
        var size = 1010;
        List<WeakReference<BigObject>> references = new ArrayList<>(size); // создаем объекты Weakreference

        for (var idx = 0; idx < size; idx++) {
            references.add(new WeakReference<>(new BigObject())); //создаем нормальный объект а затем передаем его weakreference
        }
    ...
    private static void soft() {
        ...
        var sum = 0;
        for (var idx = 0; idx < size; idx++) {
            BigObject o = references.get(idx).get();
            if (references.get(idx).get() != null) {
                // ----gc();
    ...
    private static void phantom() throws InterruptedException {
        ...
        ReferenceQueue<BigObject> refQueue = new ReferenceQueue<>(); // на самом деле там будет лежать не нащ объект

        //создаем Phantom Reference на объект типа BigObject и "подвязываем" ее на переменную a.
        PhantomReference<BigObject> phantomA = new PhantomReference<>(a, refQueue); //связыввается объект а с очередью
